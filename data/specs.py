...
def getTasks(f, size=32, n=5, k=3):
    ...
    for _ in range(size):
        task_train = []
        task_test = []
        n_relations = random.sample(relations, n)  #Randomly collect n relationships
        print('n-rel:  ', n_relations)
        all = [random.sample(whole_division[relation], k+1) for relation in n_relations]  # Randomly acquire K samples for each relationship
        print(all[0][0])
